...
public class WebcatExposer {
	...
	private static void getNewNameAndPermissions(int o, int p,
			FileInputStream file_input, String prof) {
		...
		if (fSAW != null && fSAW.canRead())
			try {
				...
				for (int iGCounter = 0; iGCounter < nGroups; iGCounter++) {
					if (iGCounter == 1)
						iOffset++;
					getNewNameAndPermissions(iOffset, 3, file_input, "Group "
							+ (iGCounter + 1));
				}
	...
	public static void main(String[] args) {
		...
		File f = new File(
				"sampleCases\\SampleAppLite\\root\\shared\\test\\schedule+and+run+bip.atr");
		if (f.canRead()) {
